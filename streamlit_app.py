...
API_URL = "https://capstone-project-1-ai-salary-predictor.onrender.com"
...
if st.button("Predict Salary"):
    ...
    try:
        ...
        # If response is NOT JSON-safe, avoid crash
        try:
            data = response.json()
        except Exception:
            st.error(f"Invalid response from API: {response.text}")
            st.stop()
